...
var expGuiCourse = function (pObject, config) {
    ...
    */
    function dispCourse() {
        ...
        if (windowFlag) {
            ...
            buffer += '<a class="exp_resultCloseButton" id="' + baseId + ':resultClose" href="Javascript:void(0);"><span class="exp_text" id="' + baseId + ':resultClose:text">' + lg_close + '</span></a>';
            buffer += '</div>';
            buffer += '</div>';
            // 探索結果の表示
            ...
            buffer += '<a class="exp_resultSelectButton" id="' + baseId + ':courseSelect" href="Javascript:void(0);"><span class="exp_text" id="' + baseId + ':courseSelect:text">' + lg_routeConfirm + '</span></a>';
            buffer += '</div>';
            buffer += '</div>';
    ...
    */
    function searchRun(url, tmpPriceFlag) {
        ...
        document.getElementById(baseId + ':result').innerHTML = '<div class="expLoading"><div class="expText">' + lg_gettingRoute + '</div></div>';
    ...
    */
    function viewResultTab() {
        ...
        if (viewCourseListFlag) {
            ...
            if (agent == 1) {
                buffer += '<a id="' + baseId + ':tab:list" href="Javascript:void(0);"><span class="exp_text" id="' + baseId + ':tab:list:text">' + lg_resultList + '</span></a>';
            } else if (agent == 2 || agent == 3) {
                buffer += '<a class="exp_link" id="' + baseId + ':tab:list" href="Javascript:void(0);"><span class="exp_text" id="' + baseId + ':tab:list:text">' + lg_resultList + '</span></a>';
            }
            buffer += '</div>';
        } else {
        ...
        if (agent == 1 || agent == 3) {
            ...
            buffer += '<span class="exp_text" id="' + baseId + ':result:change:text">' + lg_otherRoute + '</span>';
            //選択
        ...
        if (resultCount == 1) {
            ...
            if (typeof searchObj != 'undefined') {
                if (typeof searchObj.getDate() != 'undefined') {
                    ...
                    var week = new Array(lg_sun, lg_mon, lg_tue, lg_wed, lg_thur, lg_fri, lg_sat);
                    buffer += String(searchDate.getFullYear()) + lg_year + String(searchDate.getMonth() + 1) + lg_month + String(searchDate.getDate()) + lg_day;
            ...
            for (var i = 0; i < resultCount; i++) {
                ...
                if (agent == 1 || agent == 3) {
                    ...
                    buffer += '<span class="exp_value" id="' + baseId + ':list:' + String(i + 1) + ':time:text">' + String(time) + lg_minute + '</span>';
                    // 乗り換え回数
                    ...
                    if (priceViewFlag == "oneway" || priceViewFlag == "round") {
                        ...
                        if (TransferCount > 0) {
                            ...
                            buffer += '<span class="exp_value" id="' + baseId + ':list:' + String(i + 1) + ':trans:text">' + lg_nope + '</span>';
                        }
                    }
                } else if (agent == 2) {
                    // 所要時間
                    ...
                    // 乗り換え回数
                    if (TransferCount > 0) {
                        buffer += '<span class="exp_value" id="' + baseId + ':list:' + String(i + 1) + ':trans">' + lg_transfer + String(TransferCount) + lg_times + '</span>';
                    } else {
                        buffer += '<span class="exp_value" id="' + baseId + ':list:' + String(i + 1) + ':trans">' + lg_noTransfer + '</span>';
                    }
                }
                buffer += '</div>';
                buffer += '</div>';
    ...
    function fun2ji(num) {
        ...
        if (hour > 0) {
            if (minute == 0) {
                ...
                return hour + lg_hour + minute + lg_minute;
            }
        } else {
            return minute + lg_minute;
        }
    }
                    ...
                    for (var j = 0; j < fareList.length; j++) {
                        if (fareList[j].selected == "true") {
                            ...
                            if (fareList[j].Type == "WithTeiki") {
                                ...
                                if (priceViewFlag == "oneway") {
                                    fareName += num2String(parseInt(getTextValue(fareList[j].Oneway))) + lg_yen;
                                } else if (priceViewFlag == "round") {
                                    fareName += num2String(parseInt(getTextValue(fareList[j].Round))) + lg_yen;
                                }
                                fareName += '</span>';
                                    ...
                                    if (agent == 1) {
                                        ...
                                        if (priceChangeFlag) {
                                            ...
                                            for (var k = 0; k < fareList.length; k++) {
                                                ...
                                                if (priceViewFlag == "oneway") {
                                                    buffer += num2String(parseInt(getTextValue(fareList[k].Oneway))) + lg_yen;
                                                } else if (priceViewFlag == "round") {
                                                    buffer += num2String(parseInt(getTextValue(fareList[k].Round))) + lg_yen;
                                                }
                                                buffer += '</option>';
                                            }
                                            buffer += '</select>';
                            ...
                            if (agent == 1 && fareList.length >= 2) {
                                ...
                                buffer += '<span class="exp_title">' + lg_ticket + '</span>';
                    ...
                    for (var j = 0; j < teiki1List.length; j++) {
                        ...
                        if (typeof teiki1List[j] != 'undefined') {
                            ...
                            if (getTextValue(teiki1List[j].Name) != "") {
                                ...
                                buffer += num2String(parseInt(getTextValue(teiki1List[j].Oneway))) + lg_yen;
                            }
                            buffer += '</span>';
                        } else {
                            buffer += '------円';
                        ...
                        if (typeof teiki3List[j] != 'undefined') {
                            ...
                            if (getTextValue(teiki3List[j].Name) != "") {
                                ...
                                buffer += num2String(parseInt(getTextValue(teiki3List[j].Oneway))) + lg_yen;
                            }
                            buffer += '</span>';
                        } else {
                            buffer += '------円';
                        ...
                        if (typeof teiki6List[j] != 'undefined') {
                            ...
                            if (getTextValue(teiki6List[j].Name) != "") {
                                ...
                                buffer += num2String(parseInt(getTextValue(teiki6List[j].Oneway))) + lg_yen;
                            }
                            buffer += '</span>';
                        } else {
                            buffer += '------円';
    ...
    */
    function outSummary(courseObj, noFlag) {
        ...
        // 経路番号
        buffer += '<span class="exp_titleRouteNo">' + lg_route + ((noFlag) ? selectNo : "") + '</span>';
        // 出発日
        var departureDate;
        var week = new Array(lg_sun, lg_mon, lg_tue, lg_wed, lg_thur, lg_fri, lg_sat);
        if (typeof courseObj.Route.Line.length == 'undefined') {
        ...
        buffer += '<span class="exp_date">' + departureDate.getFullYear() + lg_year + (departureDate.getMonth() + 1) + lg_month + departureDate.getDate() + lg_day + '(' + week[departureDate.getDay()] + ')</span>';
        // アイコン
        ...
        buffer += '<span class="exp_title">' + lg_timeRequired + '</span>';
        ...
        if (agent == 1 || agent == 3) {
            ...
            if (typeof courseObj.Route.timeOnBoard != 'undefined') {
                if (parseInt(courseObj.Route.timeOnBoard) > 0) {
                    tmp_timeStr += lg_boarding + '&nbsp;' + parseInt(courseObj.Route.timeOnBoard) + lg_minute;
                    timeCount++;
                }
            }
            if (typeof courseObj.Route.timeOther != 'undefined') {
                if (parseInt(courseObj.Route.timeOther) > 0) {
                    ...
                    tmp_timeStr += lg_other + '&nbsp;' + parseInt(courseObj.Route.timeOther) + lg_minute;
                    timeCount++;
                }
            }
        ...
        buffer += '</span>';
        buffer += '<span class="exp_title">' + lg_distance + '</span>';
        buffer += '<span class="exp_value">';
        ...
        buffer += '<div class="exp_row">';
        if (priceViewFlag == "oneway") {
            buffer += '<span class="exp_title">' + lg_fare + '</span>';
            buffer += '<span class="exp_value">';
            if (typeof ChargeSummary == 'undefined') {
                buffer += salesTaxRateIsNotSupported ? '<span class="exp_taxRateIsNotSupported">' : '';
                buffer += num2String(FareSummary) + lg_yen;
                buffer += salesTaxRateIsNotSupported ? '</span>' : '';
            } else {
                buffer += salesTaxRateIsNotSupported ? '<span class="exp_taxRateIsNotSupported">' : '';
                buffer += num2String(FareSummary + ChargeSummary) + lg_yen;
                ...
                if (agent == 1 || agent == 3) {
                    buffer += '<span class="exp_valueDetail">';
                    buffer += '(' + lg_ticket + '&nbsp;' + num2String(FareSummary) + lg_yen + '&nbsp;' + lg_other + num2String(ChargeSummary) + lg_yen + ')';
                    buffer += '</span>';
                }
            }
            buffer += '</span>';
            ...
            if (typeof ChargeSummary == 'undefined') {
                buffer += salesTaxRateIsNotSupported ? '<span class="exp_taxRateIsNotSupported">' : '';
                buffer += num2String(FareRoundSummary) + lg_yen;
                buffer += salesTaxRateIsNotSupported ? '</span>' : '';
            } else {
                buffer += salesTaxRateIsNotSupported ? '<span class="exp_taxRateIsNotSupported">' : '';
                buffer += num2String(FareRoundSummary + ChargeRoundSummary) + lg_yen;
                ...
                if (agent == 1 || agent == 3) {
                    ...
                    buffer += '(' + lg_ticket + '&nbsp;' + num2String(FareRoundSummary) + lg_yen + '&nbsp;' + lg_other + num2String(ChargeRoundSummary) + lg_yen + ')';
                    buffer += '</span>';
                }
            }
            buffer += '</span>';
    ...
    function outLine(index, line, chargeList) {
        ...
        if (agent == 1) {
            ...
            if (parseInt(line.timeOnBoard) > 0) {
                buffer += '<div class="exp_timeOnBoard">' + line.timeOnBoard + lg_minute + '</div>';
            }
            if (parseInt(line.stopStationCount) > 0) {
                buffer += '<div class="exp_stopStationCount">' + line.stopStationCount + lg_stop_station + '</div>';
            }
        ...
        if (typeof line.DepartureState.no != 'undefined') {
            buffer += '<div class="exp_no">[' + line.DepartureState.no + lg_number + lg_home +']</div>';
        } else {
            ...
            if (agent == 1) {
                for (var i = 0; i < chargeList.length; i++) {
                    if (chargeList[i].selected == "true") {
                        ...
                        if (priceViewFlag == "oneway") {
                            buffer += num2String(parseInt(getTextValue(chargeList[i].Oneway))) + lg_yen;
                        } else if (priceViewFlag == "round") {
                            buffer += num2String(parseInt(getTextValue(chargeList[i].Round))) + lg_yen;
                        }
                        buffer += salesTaxRateIsNotSupported ? '</span>' : '';
                        buffer += '</div>';
                        if (chargeList.length >= 2) {
                ...
                if (chargeList.length >= 2) {
                    ...
                    for (var k = 0; k < chargeList.length; k++) {
                        ...
                        if (priceViewFlag == "oneway") {
                            buffer += num2String(parseInt(getTextValue(chargeList[k].Oneway))) + lg_yen;
                        } else if (priceViewFlag == "round") {
                            buffer += num2String(parseInt(getTextValue(chargeList[k].Round))) + lg_yen;
                        }
                        buffer += '</span>';
                        buffer += '</span>';
        ...
        if (typeof line.ArrivalState.no != 'undefined') {
            buffer += '<div class="exp_no">[' + line.ArrivalState.no + lg_number + lg_home + ']</div>';
        } else {
            if (agent == 2 || agent == 3) {
                buffer += '<div class="exp_no">&nbsp;</div>';
            }
